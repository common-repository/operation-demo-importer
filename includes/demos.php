		...
		*/
		...
		public function ajax_after_import() {
			...
			if ( $_POST['wpop_import_is_xml'] === 'true' ) {
				...
				if ( ! empty( $query_homepage->post ) ) {
					$home_page = $query_homepage->post;
				} else {
					$home_page = null;
				}
				...
				if ( ! empty( $query_blogpage->post ) ) {
					$blog_page = $query_blogpage->post;
				} else {
					$blog_page = null;
				}
